...
var CalendarService = rewire('../services/CalendarService');

describe('Administer Calendar Test', function CalendarTestSuite() {
  var calendar = CalendarService.__get__('calendar');
  var listSpy = sinon.spy(CalendarService.__get__('listEvents'));
  ...
  beforeEach(function setUp(done) {
    ...
    CalendarService.__set__('AdministerJWT', jwtMock);
    done();
  });

  it('should send an update to Google Calendar', function UserListParams(done) {
    var updateEvent = CalendarService.__get__('updateEvent');
  ...
  it('should return a sync token', function syncTokenTest(done) {
    ...
    var revert = CalendarService.__set__('getFullSync', mockFullSync);

    var getSyncToken = CalendarService.__get__('getSyncToken');
