...
function createJWT(scope) {
  var jwtClient = new google.auth.JWT(key.client_email, null, key.private_key, scope, config.authorizeAdmin);
...
function authorize(jwtClient) {
  ...
  return new Promise(function createJWTpromise(resolve, reject) {
    jwtClient.authorize(function authorizeJwtResponse(error, tokens) {
      if (error) {
        reject(error);
      } else {
        resolve(jwtClient);
